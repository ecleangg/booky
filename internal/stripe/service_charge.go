...
func (s *Service) handlePaymentIntentSucceeded(ctx context.Context, evt Event) (ingestResult, error) {
	var paymentIntent PaymentIntent
	if err := json.Unmarshal(evt.Data.Object, &paymentIntent); err != nil {
		return ingestResult{}, fmt.Errorf("decode payment intent from event: %w", err)
	}
	...
	if fullPI.LatestCharge.ID == "" {
		sessions, raws, err := s.Client.ListCheckoutSessionsByPaymentIntent(ctx, fullPI.ID)
		if err != nil {
			return ingestResult{}, err
		}
...
func (s *Service) handleCharge(ctx context.Context, evt Event) (ingestResult, error) {
	var eventCharge Charge
	if err := json.Unmarshal(evt.Data.Object, &eventCharge); err != nil {
		return ingestResult{}, fmt.Errorf("decode charge from event: %w", err)
	}
	...
	saleFacts, err := s.buildChargeFacts(evt, charge, bt, taxCase)
	if err != nil {
		return ingestResult{}, err
	}
...
func (s *Service) handleRefundedCharge(ctx context.Context, evt Event) (ingestResult, error) {
	var eventCharge Charge
	if err := json.Unmarshal(evt.Data.Object, &eventCharge); err != nil {
		return ingestResult{}, fmt.Errorf("decode refunded charge: %w", err)
	}
	charge, chargeRaw, err := s.Client.GetCharge(ctx, eventCharge.ID)
	if err != nil {
		return ingestResult{}, err
	...
	if settledGross == 0 && strings.ToUpper(charge.Currency) != "SEK" {
		...
		return ingestResult{Snapshots: snapshots, BalanceTxs: []domain.BalanceTransaction{primaryBT}, TaxCases: []domain.TaxCase{taxCase}, TaxCaseObjects: objects, Facts: []domain.AccountingFact{obs}}, nil
	}
	...
	for _, refund := range charge.Refunds.Data {
		...
		snapshots = append(snapshots, domain.ObjectSnapshot{ObjectType: "refund", ObjectID: refund.ID, Livemode: evt.Livemode, Payload: rawJSON(refund)})
...
func (s *Service) buildChargeFacts(evt Event, charge Charge, bt domain.BalanceTransaction, taxCase domain.TaxCase) ([]domain.AccountingFact, error) {
	...
	resolution := accounting.ResolveTaxCase(s.Config, taxCase, settledGrossSEKOre(bt), explicitVATFromTaxCase(taxCase, bt))
	if resolution.ReviewReason != "" {
		return s.buildChargeReviewFacts(evt, charge, bt, &taxCase.ID, resolution.ReviewReason)
	}
	...
	if !ok {
		return s.buildChargeReviewFacts(evt, charge, bt, &taxCase.ID, fmt.Sprintf("no Stripe balance account mapping for settlement currency %s", settlementCurrency))
	}
...
func (s *Service) buildChargeReviewFacts(evt Event, charge Charge, bt domain.BalanceTransaction, taxCaseID *uuid.UUID, reason string) ([]domain.AccountingFact, error) {
	sourceCurrency := strings.ToUpper(charge.Currency)
	settlementCurrency := strings.ToUpper(bt.Currency)
	stripeBalanceAccount, ok := s.Config.Accounts.StripeBalanceByCurrency[settlementCurrency]
	if !ok {
