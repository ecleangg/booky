...
func (s *Service) handleDisputeCreated(ctx context.Context, evt Event) (ingestResult, error) {
	var dispute Dispute
	if err := json.Unmarshal(evt.Data.Object, &dispute); err != nil {
		return ingestResult{}, fmt.Errorf("decode dispute: %w", err)
	}
	...
	if amountSEK == 0 && sourceCurrency != "SEK" {
		...
		return ingestResult{Snapshots: snapshots, BalanceTxs: balanceTransactions, Facts: []domain.AccountingFact{obs}}, nil
	}
	if len(balanceTransactions) == 0 && sourceCurrency != "SEK" {
		...
		return ingestResult{Snapshots: snapshots, Facts: []domain.AccountingFact{obs}}, nil
	}
	...
	if !ok {
		...
		return ingestResult{Snapshots: snapshots, BalanceTxs: balanceTransactions, Facts: reviewFacts}, nil
	}
	...
	return ingestResult{Snapshots: snapshots, BalanceTxs: balanceTransactions, Facts: facts}, nil
}

func (s *Service) handlePayout(ctx context.Context, evt Event) (ingestResult, error) {
	var eventPayout Payout
	if err := json.Unmarshal(evt.Data.Object, &eventPayout); err != nil {
		return ingestResult{}, fmt.Errorf("decode payout from event: %w", err)
	}
	...
	if balanceTxID == "" {
		return ingestResult{}, fmt.Errorf("payout %s missing balance transaction", payout.ID)
	}
	...
	if !ok {
		if settledGrossSEKOre(bt) == 0 && strings.ToUpper(payout.Currency) != "SEK" {
			...
			return ingestResult{Snapshots: []domain.ObjectSnapshot{{ObjectType: "payout", ObjectID: payout.ID, Livemode: evt.Livemode, Payload: payoutRaw}, {ObjectType: "balance_transaction", ObjectID: bt.ID, Livemode: evt.Livemode, Payload: btRaw}}, BalanceTxs: []domain.BalanceTransaction{bt}, Facts: []domain.AccountingFact{obs}}, nil
		}
		reviewFacts, err := accounting.BuildReviewTransferFacts(accounting.ReviewTransferInput{
			BokioCompanyID:             s.Config.Bokio.CompanyID,
			StripeAccountID:            stripeAccountID(evt),
		...
		return ingestResult{Snapshots: []domain.ObjectSnapshot{{ObjectType: "payout", ObjectID: payout.ID, Livemode: evt.Livemode, Payload: payoutRaw}, {ObjectType: "balance_transaction", ObjectID: bt.ID, Livemode: evt.Livemode, Payload: btRaw}}, BalanceTxs: []domain.BalanceTransaction{bt}, Facts: reviewFacts}, nil
	}
	...
	return ingestResult{Snapshots: []domain.ObjectSnapshot{{ObjectType: "payout", ObjectID: payout.ID, Livemode: evt.Livemode, Payload: payoutRaw}, {ObjectType: "balance_transaction", ObjectID: bt.ID, Livemode: evt.Livemode, Payload: btRaw}}, BalanceTxs: []domain.BalanceTransaction{bt}, Facts: facts}, nil
}
